...
client.on('message', message => {//Toxic Codes
    if(message.content.startsWith(prefix + "antijoin on")) {//Toxic Codes
        if(!message.channel.guild) return message.reply('**This Command Only For Servers**');//Toxic Codes
        if(!message.member.hasPermission('MANAGE_GUILD')) return message.channel.send('**Sorry But You Dont Have Permission** //Toxic Codes`MANAGE_GUILD`' );//Toxic Codes
antijoin[message.guild.id] = {//Toxic Codes
onoff: 'On',//Toxic Codes
}
message.channel.send(`**✅ The AntiJoin Is __𝐎𝐍__ !**`)//Toxic Codes
          fs.writeFile("./antijoin.json", JSON.stringify(antijoin), (err) => {//Toxic Codes
...
//Toxic Codes
client.on('message', message => {//Toxic Codes
    if(message.content.startsWith(prefix + "antijoin off")) {//Toxic Codes
        if(!message.channel.guild) return message.reply('**This Command Only For Servers**');//Toxic Codes
...
message.channel.send(`**⛔ The AntiJoin Is __𝐎𝐅𝐅__ !**`)
   ...
   if(message.content.startsWith(prefix + "setJoin")) {//Toxic Codes
       if(!message.channel.guild) return message.reply('**This Command Only For Servers**');//Toxic Codes
...
if(!time) return message.channel.send('Please Type The Account Created Time [Days]')//Toxic Codes
let embed = new Discord.RichEmbed()//Toxic Codes
.setTitle('**Done The AntiJoin Code Has Been Setup**')//Toxic Codes
.addField('Account Create Time:', `${room}`)//Toxic Codes
.addField('Requested By:', `${message.author}`)//Toxic Codes
.setThumbnail(message.author.avatarURL)//Toxic Codes
.setFooter(`${client.user.username}`)//Toxic Codes
message.channel.sendEmbed(embed)//Toxic Codes
antijoin[message.guild.id] = {//Toxic Codes
created: time,//Toxic Codes//Toxic Codes
onoff: 'On',//Toxic Codes
}//Toxic Codes
fs.writeFile("./antijoin.json", JSON.stringify(antijoin), (err) => {//Toxic Codes
if (err) console.error(err)//Toxic Codes
...
client.on("guildMemberAdd", async member => {//Toxic Codes
  if(!antijoin[member.guild.id]) antijoin[member.guild.id] = {//Toxic Codes
    onoff: 'Off'
  }
  if(antijoin[member.guild.id].onoff === 'Off') return;//Toxic Codes
    let accounttime = `${antijoin[member.guild.id].created}`//Toxic Codes
    let moment2 = require('moment-duration-format'),//Toxic Codes
        moment = require("moment"),//Toxic Codes
        date = moment.duration(new Date() - member.user.createdAt).format("d");//Toxic Codes
 
    if(date < accounttime) {//Toxic Codes
      member.ban(`Member account age is lower than ${antijoin[member.guild.id].created} days.`)//Toxic Codes
    }//Toxic Codes
  });//Toxic Codes
